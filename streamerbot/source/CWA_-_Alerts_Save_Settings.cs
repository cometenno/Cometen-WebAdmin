...
public class CPHInline
{
    ...
    public bool Execute()
    {
        string raw = GetArg("settingsJson", GetArg("settings", ""));
        if (string.IsNullOrWhiteSpace(raw))
        {
            Result("No settingsJson was received.", true);
            return true;
        }
        ...
        try
        {
            ...
            string compact = parsed.ToString(Formatting.None);
            CPH.SetGlobalVar(SettingsKey, compact, true);
            CPH.SetGlobalVar(LastResultKey, "Alert settings saved and sent to overlay.", true);
            Broadcast(compact, "saved");
        }
        catch (Exception ex)
        {
            Result("Invalid alert settings JSON: " + ex.Message, true);
        }
        return true;
    }
    ...
    private void Result(string text, bool error)
    {
        CPH.SetGlobalVar(LastResultKey, text, true);
        string json = JsonConvert.SerializeObject(new { source = "CometenWebAdmin", type = "ALERTS_RESULT", result = text });
        CPH.WebsocketBroadcastString(json);
        if (error) CPH.LogError("[CWA Alerts] " + text); else CPH.LogInfo("[CWA Alerts] " + text);
    }
    ...
    private void Broadcast(string settingsJson, string action)
    {
        string json = JsonConvert.SerializeObject(new {
            ...
            lastResult = "Alert settings saved and sent to overlay."
        });
        CPH.WebsocketBroadcastString(json);
    }
}
