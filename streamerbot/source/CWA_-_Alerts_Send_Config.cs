...
public class CPHInline
{
    ...
    {
        string settingsJson = "";
        try { settingsJson = CPH.GetGlobalVar<string>(SettingsKey, true); } catch { }
        if (string.IsNullOrWhiteSpace(settingsJson)) settingsJson = DefaultSettingsJson;
        try { JToken.Parse(settingsJson); } catch { settingsJson = DefaultSettingsJson; }

        CPH.SetGlobalVar(SettingsKey, settingsJson, true);
        CPH.SetGlobalVar(LastResultKey, "Saved alert config sent to overlay.", true);
        string json = JsonConvert.SerializeObject(new {
            ...
            lastResult = "Saved alert config sent to overlay."
        });
