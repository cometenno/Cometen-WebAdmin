...
public class CPHInline
{
    ...
    public bool Execute()
    {
        CPH.SetGlobalVar(SettingsKey, DefaultSettingsJson, true);
        CPH.SetGlobalVar(LastResultKey, "Alert settings reset to CWA v19.0 defaults.", true);
        string json = JsonConvert.SerializeObject(new {
            source = "CometenWebAdmin",
            type = "ALERTS_SETTINGS",
            action = "reset",
            settings = JToken.Parse(DefaultSettingsJson),
            settingsJson = DefaultSettingsJson,
            lastResult = "Alert settings reset to CWA v19.0 defaults."
        });
